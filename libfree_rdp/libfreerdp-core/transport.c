...
boolean transport_tsg_connect(rdpTransport* transport, const char* hostname, uint16 port)
{
	...
	tsg->transport = transport; /* TODO tsg->transport, transport->tsg (circle ref) */
	transport->tsg = tsg;
...
boolean transport_connect(rdpTransport* transport, const char* hostname, uint16 port)
{
	if(transport->settings->tsg)
	{
		...
		return tcp_connect(transport->tcp_in, transport->settings->tsg_server, 443) && tcp_connect(transport->tcp_out, transport->settings->tsg_server, 443) && transport_tsg_connect(transport, hostname, port);
	}
	else
	{
...
{
	transport->tcp_in->sockfd = sockfd; /* FIXME not TSG conform */
}
...
boolean transport_accept_tls(rdpTransport* transport)
{
	...
	if (tls_accept(transport->tls_in, transport->settings->cert_file, transport->settings->privatekey_file) != true)
		return false;
...
int transport_read(rdpTransport* transport, STREAM* s)
{
	...
	{
		if (transport->layer == TRANSPORT_LAYER_TLS)
			status = tls_read(transport->tls_in, stream_get_tail(s), stream_get_left(s));
		else if (transport->layer == TRANSPORT_LAYER_TCP)
			status = tcp_read(transport->tcp_in, stream_get_tail(s), stream_get_left(s));
...
boolean transport_set_blocking_mode(rdpTransport* transport, boolean blocking)
{
	...
	if(transport->settings->tsg)
		tcp_set_blocking_mode(transport->tcp_out, blocking);
	return tcp_set_blocking_mode(transport->tcp_in, blocking);
}

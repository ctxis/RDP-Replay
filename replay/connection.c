...
uint8 client_random[CLIENT_RANDOM_LENGTH];//SE
void set_cli_rand(uint8 *rand)//SE
{//SE
    memcpy(client_random, rand, CLIENT_RANDOM_LENGTH) ;//SE
}//SE

//SE static boolean rdp_client_establish_keys(rdpRdp* rdp)
boolean rdp_client_establish_keys(rdpRdp* rdp)//SE
{
//SE	uint8 client_random[CLIENT_RANDOM_LENGTH];
...
//SE	memset(crypt_client_random, 0, sizeof(crypt_client_random));
//SE	crypto_nonce(client_random, sizeof(client_random));

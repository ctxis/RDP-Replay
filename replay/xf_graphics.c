...
play_new_pointer(context, pointer, ((xfPointer*) pointer)->cursor) ;//SE
	xfree(ci.pixels);
}
...
play_del_pointer(context, ((xfPointer*) pointer)->cursor) ;//SE
}
...
//SE	if (xfi->window != NULL)
//SE		XDefineCursor(xfi->display, xfi->window->handle, ((xfPointer*) pointer)->cursor);
play_set_pointer(context, ((xfPointer*) pointer)->cursor) ;//SE
}
/* Glyph Class */

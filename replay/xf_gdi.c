...
boolean xf_set_rop3(xfInfo* xfi, int rop3)
{
	...
	{
		extern int warning ;
		if (warning) printf("Warning: Unsupported ROP3: 0x%08X\n", rop3);
		function = GXand ;
...
{
//SE	printf("DrawNineGrid\n");
DRAW_NINE_GRID_ORDER *o = draw_nine_grid ;//SE
//printf("DrawNineGrid %dx%d %dx%d %u\n", o->srcLeft, o->srcTop, o->srcRight, o->srcBottom, o->bitmapId) ;//SE
}
...
if (!bitmap) { printf("ERROR: GDI: No bitmap for memblt!!\n") ; return ; }//SE
...
void se_MultiDstBlt()
{
    extern int trace_ord ;
//  if (trace_ord)
        printf("se_MultiDstBlt\n") ;
}

void xf_gdi_register_update_callbacks(rdpUpdate* update)
{
	rdpPrimaryUpdate* primary = update->primary;
void play_SaveBitmap(rdpContext*, SAVE_BITMAP_ORDER*) ;//SE
void play_MultiScrBlt(rdpContext*, MULTI_SCRBLT_ORDER*) ;//SE
void play_MultiDstBlt(rdpContext*, MULTI_DSTBLT_ORDER*) ;//SE
void play_MultiPatBlt(rdpContext*, MULTI_PATBLT_ORDER*) ;//SE
...
//SE	primary->DrawNineGrid = NULL;
primary->DrawNineGrid = xf_gdi_draw_nine_grid;//SE
//SE	primary->MultiDstBlt = NULL;
//SE	primary->MultiPatBlt = NULL;
//SE	primary->MultiScrBlt = NULL;
primary->MultiDstBlt = play_MultiDstBlt ;//SE
primary->MultiPatBlt = play_MultiPatBlt ;//SE
primary->MultiScrBlt = play_MultiScrBlt ;//SE
...
//SE	primary->SaveBitmap = NULL;
//SE	primary->GlyphIndex = NULL;
//SE	primary->FastIndex = NULL;
//SE	primary->FastGlyph = NULL;
primary->SaveBitmap = play_SaveBitmap ;//SE
primary->GlyphIndex = se_GlyphIndex ;//SE
primary->FastIndex  = se_FastIndex ;//SE
primary->FastGlyph  = se_FastGlyph ;//SE

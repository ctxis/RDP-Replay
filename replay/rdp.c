...
//if (compressed_type) printf(">>> RDP: Compressed_type=%02x\n", (int)compressed_type) ;//SE
...
if (stream_get_left(s)<5) printf("rdp_recv_out_of_sequence_pdu: NO DATA  BAD BAD BAD!!!\n") ;//SE
	rdp_read_share_control_header(s, &length, &type, &channelId);
if (length - 2 > stream_get_left(s)) printf("NO DATA#2  BADBADBADBAD len=%d left=%d\n", (int)length, (int)stream_get_left(s)) ;//SE
...
//se_out("  Decrypted: ", s->p, s->data+s->size) ;//SE
		}
	...
	{
if (stream_get_left(s)>8 && (s->p[6]/*&0x20*/))//SE
{//SE
   uint32 roff,rlen;//SE
 //printf("rdp_recv_tpkt_pdu: compFlags: %02x  size=%d\n", s->p[6], (int)stream_get_left(s)-8) ;//SE
   decompress_rdp(rdp, s->p+8, stream_get_left(s)-8, s->p[6], &roff, &rlen) ;//SE
 //printf("... decom-len = %d\n", rlen) ;
}//SE
...
//printf("GOT GLOBAL cmd: type=%02x(DATA=07) src=%d\n", (int)pduType, (int)pduSource) ; fflush(stdout) ;//SE
...
//SE static boolean rdp_recv_callback(rdpTransport* transport, STREAM* s, void* extra)
boolean rdp_recv_callback(rdpTransport* transport, STREAM* s, void* extra)//SE
{
	rdpRdp* rdp = (rdpRdp*) extra;

//printf("<< REPLY: rdp_recv_callback: rdp-state=%d,len=%d: ", rdp->state, (int)stream_get_left(s)) ;//SE
//se_out("rdp_recv_callback-Data:",s->p,s->p+4);//SE
	switch (rdp->state)
	{
		...
		case CONNECTION_STATE_LICENSE:
                        if (15==stream_get_left(s) && (s->p[7]&0xfc)==0x3c) break ; //SE Skip extra ChannelJoinConfirm messages

...
if (trace_ord) se_out("FastOrders: ",s->p, s->data+s->size) ;//SE

if (stream_get_left(s)<2) printf("ERROR: fastpath_recv_orders: NOT ENOUGH DATA FOR numberOrders!!\n") ;//SE
//se_out("Orders:",s->p,s->data+s->size) ;//SE
	stream_read_uint16(s, numberOrders); /* numberOrders (2 bytes) */
//printf("fastpath_recv_orders: numberOrders=%d", (int)numberOrders) ;//SE
//printf(" stream has %d bytes.\n", (int)stream_get_left(s)) ;//SE
//fflush(stdout) ;//SE
...
extern int trace_ord ;//SE
uint8_t *oldp = s->p ;//SE
//printf("BytesLeft: %d: ",(int)(s->data+s->size-s->p)) ;//SE
if (stream_get_left(s)<1) { printf("ERROR: fastpath_recv_orders No DATA!! OrdersLeft:%d\n",(int)numberOrders) ; return true ; }//SE
		if (!update_recv_order(update, s))
			return false;
		numberOrders--;
if (trace_ord) se_out(" ...^^ order: ", oldp, s->p) ;//SE
	}
...
{
// SE	stream_seek_uint16(s); /* size (2 bytes), must be set to zero */
}
...
if (0) { printf("fastpath_recv_update: Code=%d len=%d (%s)", (int)updateCode, (int)size,
       updateCode<12 ? unam[updateCode] : "Bad"); se_out(": ",s->p,s->p+size) ;}
...
//SE	uint32 roff;
	...
	stream_read_uint16(s, size);
        if (stream_get_left(s)<size)//SE
        {//SE
            printf("ERROR: Bad length (too big) in fastpath_recv_update_data\n") ;//SE
            stream_set_pos(s, stream_get_pos(s) + stream_get_left(s));//SE
            return true;//SE
        }//SE
	...
	{
		uint8 *SE_decompress_rdp(rdpRdp*, uint8*, int, int, uint32*) ;//SE
		uint8 *newp = SE_decompress_rdp(rdp, s->p, size, compressionFlags, &rlen) ;//SE
//SE		if (decompress_rdp(rdp, s->p, size, compressionFlags, &roff, &rlen))
		if (newp)//SE
		{
			comp_stream = stream_new(0);
//SE			comp_stream->data = rdp->mppc->history_buf + roff;
			comp_stream->data = newp ; //SE
			comp_stream->p = comp_stream->data;
			comp_stream->size = rlen;
			size = comp_stream->size;
//se_out("Decompressed: ", comp_stream->data, comp_stream->data+rlen) ;//SE
		}
		else
		{
			extern int warning ;//SE
			if (warning) printf("SE_decompress_rdp() failed\n");//SE
//SE			stream_seek(s, size);
			stream_set_pos(s, next_pos) ;//SE
			return false ;//SE
		}
	}
...
printf("ERROR: fastpath_recv_update_data failed!\n") ;//SE
IFCALL(update->EndPaint, update->context);//SE
			return false;
		}
if (stream_get_left(s)<0) { printf("ERROR: fastpath_recv_update_data: Ran out of data on last call!!\n") ; break ; }//SE
	}
if (stream_get_left(s)) printf("ERROR: fastpath_recv_updates: bytes left=%d\n", (int)stream_get_left(s)) ;//SE

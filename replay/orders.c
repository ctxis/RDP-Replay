...
{
//printf("update_read_memblt_order: flags=0x%x\n",(int)(orderInfo->fieldFlags)) ;//SE
	if (orderInfo->fieldFlags & ORDER_FIELD_01)
{//SE
		stream_read_uint16(s, memblt->cacheId);
memblt->colorIndex = (memblt->cacheId >> 8);//SE
memblt->cacheId = (memblt->cacheId & 0xFF);//SE
}//SE
...
//SE	memblt->colorIndex = (memblt->cacheId >> 8);
//SE	memblt->cacheId = (memblt->cacheId & 0xFF);
}
...
mem3blt->colorIndex = (mem3blt->cacheId >> 8);//SE
mem3blt->cacheId = (mem3blt->cacheId & 0xFF);//SE
}//SE
...
//SE	mem3blt->colorIndex = (mem3blt->cacheId >> 8);
//SE	mem3blt->cacheId = (mem3blt->cacheId & 0xFF);
}
...
{
extern int trace_ord ;//SE
if (trace_ord) printf("polygon_sc: ") ;//SE
	if (orderInfo->fieldFlags & ORDER_FIELD_01)
...
{
extern int trace_ord,warning ;//SE
	NINE_GRID_BITMAP_INFO* nineGridInfo;

if (trace_ord) se_out("CreateNineGridOrder: ",s->p,s->p+23) ;//SE
...
stream_read_uint16(s, create_nine_grid_bitmap->cx) ;//SE
stream_read_uint16(s, create_nine_grid_bitmap->cy) ;//SE
if (warning) printf("WARNING: update_read_create_nine_grid_bitmap_order: Id=%d\n", (int)create_nine_grid_bitmap->bitmapId) ;//SE
...
{
extern int trace_ord, warning ;//SE
if (trace_ord) se_out("ReadNineGridBMP Flags: ", s->p, s->p+20) ;//SE
...
if (warning) printf("WARNING: update_read_stream_bitmap_first_order: Dropping %d bytes (stream has %d bytes) (total=%d)\n",//SE
 (int)stream_bitmap_first->bitmapBlockSize, (int)stream_get_left(s), (int)stream_bitmap_first->bitmapSize) ;//SE
if (trace_ord) se_out(" -> ", s->p, s->p+stream_bitmap_first->bitmapBlockSize) ;//SE
...
if (warning) printf("WARNING: update_read_stream_bitmap_next_order: Dropping %d bytes (stream has %d bytes)\n", (int)stream_bitmap_next->bitmapBlockSize, (int)stream_get_left(s)) ;//SE
if (trace_ord) se_out(" 9Grid-NxData: ", s->p, s->p+stream_bitmap_next->bitmapBlockSize) ;//SE
...
boolean update_recv_primary_order(rdpUpdate* update, STREAM* s, uint8 flags)
{
	...
	switch (orderInfo->orderType)
	{
		...
		default:
printf("ERROR: ORDER not processed: update_recv_primary_order orderType=%d\n", (int)orderInfo->orderType) ;//SE
			break;
	}
...
void update_recv_secondary_order(rdpUpdate* update, STREAM* s, uint8 flags)
{
	...
	switch (orderType)
	{
		...
		default:
printf("ORDER not processed: update_recv_secondary_order orderType=%d\n", (int)orderType) ;//SE
			break;
	}
...
void update_recv_altsec_order(rdpUpdate* update, STREAM* s, uint8 flags)
{
	...
	switch (orderType)
	{
		...
		default:
printf("ORDER not processed: update_recv_altsec_order orderType=%d\n", (int)orderType) ;//SE
			break;
	}
}
...
{
	uint8 controlFlags;
extern int was_order ; was_order = 1 ;//SE
if (stream_get_left(s)<1) { printf("ERROR: update_recv_order called with no data left!!\n") ; return false ; }//SE
//printf(" update_recv_order: (len=%d) ", (int)stream_get_left(s)) ;//SE
//se_out(": ",s->p, s->data+s->size) ;//SE
